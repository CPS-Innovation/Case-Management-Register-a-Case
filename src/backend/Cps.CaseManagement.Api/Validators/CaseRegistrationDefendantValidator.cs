...
public class CaseRegistrationDefendantValidator : AbstractValidator<CaseRegistrationDefendant>
{
    public CaseRegistrationDefendantValidator()
    {
        this.RuleFor(x => x.Surname)
            ...
            .When(x => x.IsDefendant);
        this.RuleFor(x => x.Firstname)
            .MaximumLength(CaseRegistrationDefaults.NameMaxLength)
            .Matches(CaseRegistrationInputPatterns.PersonName)
            .WithMessage($"Firstname {CaseRegistrationInputPatterns.PersonNameMessage}")
            .When(x => x.IsDefendant);
        this.RuleFor(x => x.CompanyName)
            .NotEmpty()
            .MaximumLength(CaseRegistrationDefaults.CompanyNameMaxLength)
            .When(x => !x.IsDefendant);
        ...
        this.RuleFor(x => x.JsonPropertyNameArrestSummonsNumber)
            .MaximumLength(CaseRegistrationDefaults.AsnMaxLength)
            .Matches(CaseRegistrationInputPatterns.Alphanumeric)
            .WithMessage($"ArrestSummonsNumber {CaseRegistrationInputPatterns.AlphanumericMessage}");
