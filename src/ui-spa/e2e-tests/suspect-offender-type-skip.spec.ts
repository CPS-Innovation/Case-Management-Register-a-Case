...
test("Scenario 15: type of offender can be skipped, and none is recorded for the suspect", async ({
  page,
}) => {
  ...
  const errorSummary = page.getByTestId("suspect-offender-types-error-summary");
  await expect(errorSummary).toBeVisible();
  await expect(
    errorSummary.getByRole("heading", { name: "There is a problem" }),
  ).toBeVisible();

  const selectTypeLink = page.getByTestId("suspect-offender-radio-link");
  const skipLink = page.getByTestId("suspect-detail-skip-link");
  await expect(selectTypeLink).toHaveText("Select the type of offender");
  await expect(skipLink).toHaveText("I do not have the type of offender");
  ...
  expect(
    defendants[0].type,
    "skipping the type of offender must submit an empty type",
  ).toBe("");
});
